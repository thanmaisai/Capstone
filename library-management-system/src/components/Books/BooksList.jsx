...
const BooksList = ({ books, role, onDelete, onUpdate }) => {
  ...
  // Memoize borrowedBooks to prevent unnecessary recalculations
  const borrowedBooks = useMemo(() => user?.borrowedBooks || [], [user]);

  // Memoize handleBorrow to avoid re-creating the function on every render
  const handleBorrow = useCallback(
    ...
    [borrowBookMutation, borrowBook]
  );

  // Memoize the rendering of each book item to avoid unnecessary re-renders
  const renderBook = useCallback(
    ...
    [theme, role, handleBorrow, onDelete, onUpdate, borrowedBooks]
  );

  // Render the list of books, memoized to avoid unnecessary re-renders
